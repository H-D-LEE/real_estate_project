...
# 먼저 서울 전체 합계를 계산
seoul_total_data = data[data['지역_거래규모'].str.startswith('서울_')]
if not seoul_total_data.empty:
    relevant_columns = [col for col in seoul_total_data.columns if col.startswith(('2019', '2020', '2021', '2022', '2023', '2024'))]
    seoul_total_sum = seoul_total_data[relevant_columns].sum().to_frame().T
    seoul_total_sum.insert(0, '지역_거래규모', '서울')  # '서울' 이름으로 유지
    seoul_data = pd.concat([seoul_data, seoul_total_sum], ignore_index=True)

# 각 구별로 합계를 계산
for district in seoul_districts:
    district_data = data[data['지역_거래규모'].str.contains(district)]
    if not district_data.empty:
        relevant_columns = [col for col in district_data.columns if col.startswith(('2019', '2020', '2021', '2022', '2023', '2024'))]
        district_data_sum = district_data[relevant_columns].sum().to_frame().T
        district_data_sum.insert(0, '지역_거래규모', district)  # 구 이름을 유지
        seoul_data = pd.concat([seoul_data, district_data_sum], ignore_index=True)

# 데이터프레임을 월별로 정렬
sorted_columns = ['지역_거래규모'] + sorted([col for col in seoul_data.columns if col != '지역_거래규모'])
seoul_data = seoul_data[sorted_columns]

# 데이터프레임을 CSV 파일로 저장
seoul_data.to_csv('서울 월별 합산 데이터.csv', encoding='utf-8-sig', index=False)
